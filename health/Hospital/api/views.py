...
from Hospital.api.serializers import HospitalSerializer,FileUploadSerializer,SaveFileSerializer
...
class HospitalList(APIView):
    def get(self,request):
        resturants = Hospital.objects.all()
        serializer = HospitalSerializer(resturants, many=True)
...
class HospitalDetail(APIView):
    ...
    def get(self,request,pk):
        model = get_object_or_404(Hospital, pk=pk)

        serializer = HospitalSerializer(model)
        return Response(serializer.data)
    ...
    def post(self, request, pk):
        ...
        serializer = HospitalSerializer(model, data=data, partial=True)
